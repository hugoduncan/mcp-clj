...
;; Update session ID if provided
                               ...
                               ;; Process response
                                (process-json-rpc-response client (:body response)))
                             ;; HTTP error
                              (let [error-msg (str "HTTP error: " (:status response) " - "
                                                   (or (:body response) ""))]
                                (log/error :http/request-failed {:status (:status response)
                                                                 :body (:body response)})
                                (.completeExceptionally future (ex-info error-msg
                      ...
                      (try
                        (let [url (str (:base-url client) "/mcp")
                          ...
                          (when (not= 200 (:status response))
                            (log/error :http/notification-failed {:status (:status response)
                                                                  :body (:body response)})))
                        (catch Exception e
                          (log/error :http/notification-error {:error e :method method}))))))
